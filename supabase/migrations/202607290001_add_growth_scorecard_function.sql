-- Add an aggregate-only growth scorecard for the protected Worker admin endpoint.
...
as $$
  with days as (
    select p_start_date + offset as metric_date
    from generate_series(0, p_end_date - p_start_date) as offset
  ),
  previews as (
    select
