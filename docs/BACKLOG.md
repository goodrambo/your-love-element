...
- [x] Add a golden parity test that feeds the same aggregate RPC fixtures through the protected Worker route and local adapter, requires identical range, goal/streak, totals, days, and attribution, locks deterministic multi-UTM ordering, and makes the Worker reject incomplete commerce rows plus duplicate or out-of-range funnel attribution rows without using production data.
