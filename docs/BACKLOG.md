...
- [x] Add a golden parity test that feeds the same aggregate RPC fixtures through the protected Worker route and local adapter, requires identical range, goal/streak, totals, days, and attribution, and locks deterministic multi-UTM ordering without using production data.
