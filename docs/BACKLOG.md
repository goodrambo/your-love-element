...
- [ ] Release a scorecard freshness gate that rejects aggregate input unless its range ends on the Asia/Taipei day immediately before `run_date`. Hypothesis: stale aggregates will fail closed as observability instead of producing a repeated conversion/traffic decision after a new day closes. Primary metric: stale-input rejection coverage. Guardrail: current contiguous latest-closed-day inputs continue to pass; no scorecard, authority, privacy, or paid-flow semantics change. Gate: unit regression plus Harness/CI; stop if any valid current-day fixture is rejected.
