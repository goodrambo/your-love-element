...
Collect the first seven complete Taipei measurement days for the released conversion baseline and recheck Search Console when processed data becomes available. The hourly `yle` heartbeat must continue producing one non-monitoring result per run until the early exit gate is met. Do not run a paid-flow E2E, send customer mail, publish to social channels, use Meta, or access Lemon Squeezy without separate authorization.
