...
- A shared golden fixture now passes through both the protected Worker scorecard route and local adapter, requiring identical range, goal/streak, totals, days, and deterministic attribution ordering; the Worker also fails closed on missing, duplicate, or out-of-range closed commerce days so incomplete purchase truth cannot become an evaluator input.
