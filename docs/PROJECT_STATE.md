...
- A shared golden fixture now passes through both the protected Worker scorecard route and local adapter, requiring identical range, goal/streak, totals, days, and deterministic attribution ordering; both paths reject unknown RPC aggregate fields, while the Worker also fails closed on incomplete commerce rows and duplicate or out-of-range funnel attribution rows so invalid aggregates cannot become an evaluator input.
